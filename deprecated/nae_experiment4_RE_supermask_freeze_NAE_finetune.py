...
result_dir = f'modified_results/experiments-2/experiment4/{args.run}_sparsitiy_{args.pruning_ratio}_leaveout_{args.leave}'
if not os.path.isdir(result_dir):
    os.makedirs(result_dir)
    print(f'creating {result_dir}')
shutil.copy('nae_experiment4_RE_supermask_freeze_NAE_finetune.py', f'{result_dir}/nae_experiment4_RE_supermask_freeze_NAE_finetune.py')
print(f'file copied: ', f'{result_dir}/nae_experiment4_RE_supermask_freeze_NAE_finetune.py')
...
z_dim = 17
encoder = FC_supermask_encode_nonstochastic(device = device, sparsity = args.pruning_ratio)#ConvNet2FC(1, z_dim, nh=8, nh_mlp=1024, out_activation='linear')
decoder = FC_supermask_decode_nonstochastic(device = device, sparsity = args.pruning_ratio) #DeConvNet2(z_dim, 1, nh=8, out_activation='sigmoid')
...
for i_epoch in tqdm(range(finetune_epoch)):
    for x, _ in tqdm(in_train_dl):
        x = x.reshape(-1,784).cuda(device)
        d_result = new_model.train_step_finetune(x, opt, locked_masks, clip_grad=clip_grad)
