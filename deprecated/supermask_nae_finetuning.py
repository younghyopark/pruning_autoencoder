...
result_dir = f'experiments/supermask_nae_finetune/leaveout_{args.leave}'
if not os.path.isdir(result_dir):
    os.makedirs(result_dir)
    print(f'creating {result_dir}')
shutil.copy('supermask_nae_finetuning.py', f'{result_dir}/supermask_nae_finetuning.py')
print(f'file copied: ', f'{result_dir}/supermask_nae_finetuning.py')
...
def train(model, device, optimizer, epoch, i):
    ...
    for x, _ in tqdm(in_train_dl):
        ...
        d_result = model.train_step(x, optimizer, clip_grad=None)

        writer.add_scalar('supermask_nae_finetune_{}/loss'.format(sparsity), d_result['loss'], i + 1)
        writer.add_scalar('supermask_nae_finetune_{}/energy_diff'.format(sparsity), d_result['pos_e'] - d_result['neg_e'], i + 1)
        writer.add_scalar('supermask_nae_finetune_{}/pos_e'.format(sparsity), d_result['pos_e'], i + 1)
        writer.add_scalar('supermask_nae_finetune_{}/neg_e'.format(sparsity), d_result['neg_e'], i + 1)
        writer.add_scalar('supermask_nae_finetune_{}/z_norm'.format(sparsity), d_result['z_norm'], i + 1)
        writer.add_scalar('supermask_nae_finetune_{}/z_neg_norm'.format(sparsity), d_result['z_neg_norm'], i + 1)
        writer.add_scalar('supermask_nae_finetune_{}/encoder_l2'.format(sparsity), d_result['encoder_norm'], i + 1)
        writer.add_scalar('supermask_nae_finetune_{}/decoder_l2'.format(sparsity), d_result['decoder_norm'], i + 1)
        ...
        writer.add_image('supermask_nae_finetune_{}/sample'.format(sparsity), img_grid, i + 1)
        ...
        if i% 50==0:
            ...
            writer.add_scalar('supermask_nae_finetune_{}/auroc'.format(sparsity), auc, i + 1)
            writer.add_scalar('supermask_nae_finetune_{}/val_error'.format(sparsity), val_err.mean().item(), i + 1)
        ...
        if i % 200 == 0:
            ...
            writer.add_image('supermask_nae_finetune_{}/inlier_recon'.format(sparsity), img_grid, i+1)
            ...
            x_and_recon = torch.cat([test_data, recon])
            img_grid = make_grid(x_and_recon.detach().cpu(), nrow=10, range=(0, 1))
            writer.add_image('supermask_nae_finetune_{}/outier_recon'.format(sparsity), img_grid, i+1)
...
def main():
    ...
    torch.save(model.state_dict(), f"{result_dir}/supermask_nae_finetuned_{sparsity}.pkl")
    
            
    in_pred = predict(model, in_test_dl, device, False)
    out_pred = predict(model, out_test_dl, device, False)
    auc = roc_btw_arr(out_pred, in_pred)
    writer.add_scalar('supermask_nae_finetune_/AUROC', auc, index + 1)
    writer.add_scalar('supermask_nae_finetune_/IND_RE_mean', torch.mean(in_pred), index)
    writer.add_scalar('supermask_nae_finetune_/OOD_RE_mean', torch.mean(out_pred), index)
    writer.add_scalar('supermask_nae_finetune_/pruning_ratio',sparsity,index)
    ...
    x_and_recon = torch.cat([test_data, recon])
    img_grid = make_grid(x_and_recon.detach().cpu(), nrow=10, range=(0, 1))
    writer.add_image('supermask_nae_finetune_/inlier_recon', img_grid, index)
    ...
    x_and_recon = torch.cat([test_data, recon])
    img_grid = make_grid(x_and_recon.detach().cpu(), nrow=10, range=(0, 1))
    writer.add_image('supermask_nae_finetune_/outlier_recon', img_grid, index)
