...
result_dir = f'experiments/supermask/leaveout_{args.leave}'
...
def train(model, device, optimizer, epoch, i):
    ...
    for x, _ in tqdm(in_train_dl):
        ...
        if i%50==0:
            ...
            writer.add_scalar('supermask_{}/loss'.format(sparsity), d_result['loss'], i + 1)
            writer.add_scalar('supermask_{}/z_norm'.format(sparsity), d_result['z_norm'], i + 1)
            writer.add_scalar('supermask_{}/recon_error'.format(sparsity), d_result['recon_error'], i + 1)
            writer.add_scalar('supermask_{}/encoder_l2'.format(sparsity), d_result['encoder_norm'], i + 1)
            writer.add_scalar('supermask_{}/decoder_l2'.format(sparsity), d_result['decoder_norm'], i + 1)
        ...
        if i % 200 == 0:
            ...
            writer.add_scalar('supermask_{}/auroc'.format(sparsity), auc, i + 1)
            writer.add_scalar('supermask_{}/val_error'.format(sparsity), val_err.mean().item(), i + 1)
...
def main():
    ...
    z_dim = 17
    encoder = Supermask_ConvNet2FC(1, z_dim, nh=8, nh_mlp=1024, out_activation='linear')
    ...
    torch.save(model.state_dict(), f"{result_dir}/supermask_{sparsity}.pkl")
    
            
    in_pred = predict(model, in_test_dl, device, False)
    out_pred = predict(model, out_test_dl, device, False)
    auc = roc_btw_arr(out_pred, in_pred)
    writer.add_scalar('supermask/AUROC', auc, index + 1)
    writer.add_scalar('supermask/IND_RE_mean', torch.mean(in_pred), index)
    writer.add_scalar('supermask/OOD_RE_mean', torch.mean(out_pred), index)
    writer.add_scalar('supermask/pruning_ratio',sparsity,index)
    ...
    x_and_recon = torch.cat([test_data, recon])
    img_grid = make_grid(x_and_recon.detach().cpu(), nrow=10, range=(0, 1))
    writer.add_image('supermask/inlier_recon', img_grid, index)
    ...
    x_and_recon = torch.cat([test_data, recon])
    img_grid = make_grid(x_and_recon.detach().cpu(), nrow=10, range=(0, 1))
    writer.add_image('supermask/outlier_recon', img_grid, index)
