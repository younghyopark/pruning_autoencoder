...
result_dir = f'modified_results/experiments/experiment6/{args.run}_sparsitiy_{args.pruning_ratio}_leaveout_{args.leave}'
if not os.path.isdir(result_dir):
    os.makedirs(result_dir)
    print(f'creating {result_dir}')
shutil.copy('nae_experiment6_RE_weight_NAE_supermask_freeze_NAE_finetune.py', f'{result_dir}/nae_experiment6_RE_weight_NAE_supermask_freeze_NAE_finetune.py')
print(f'file copied: ', f'{result_dir}/nae_experiment6_RE_weight_NAE_supermask_freeze_NAE_finetune.py')
...
'''Convert to finetuning model'''
encoder = FC_original_encode(device = device, previous_model = new_model)
decoder = FC_original_decode(device = device, previous_model = new_model)

new_model_2 = NAE(encoder, decoder, l2_norm_reg=l2_norm_reg, l2_norm_reg_en=l2_norm_reg_en, spherical=spherical, z_step=10, z_stepsize=0.2, z_noise_std=0.05, x_step=50, x_stepsize=0.2, x_noise_std=0.05, x_noise_anneal=1., x_bound=(0, 1), z_bound=None, x_clip_langevin_grad=None)

new_model_2.cuda(device)

print(new_model_2)


opt = Adam(new_model_2.parameters(), lr=0.00001)

print('starting NAE Finetuning with frozen weights...')
l_ae_result = []

locked_masks = dict()
for sub_network in new_model.children():
    for layers in sub_network.children():
        locked_masks[layers.mask.shape]=layers.mask==0
print(locked_masks)

i = 0
for i_epoch in tqdm(range(n_finetune_epoch)):
    for x, _ in tqdm(in_train_dl):
        x = x.reshape(-1,784).cuda(device)
        d_result = new_model_2.train_step_finetune(x, opt, locked_masks, clip_grad=clip_grad)

        writer.add_scalar('step3/loss', d_result['loss'], i + 1)
        writer.add_scalar('step3/energy_diff', d_result['pos_e'] - d_result['neg_e'], i + 1)
        writer.add_scalar('step3/pos_e', d_result['pos_e'], i + 1)
        writer.add_scalar('step3/neg_e', d_result['neg_e'], i + 1)
        writer.add_scalar('step3/z_norm', d_result['z_norm'], i + 1)
        writer.add_scalar('step3/z_neg_norm', d_result['z_neg_norm'], i + 1)
        writer.add_scalar('step3/encoder_l2', d_result['encoder_norm'], i + 1)
        writer.add_scalar('step3/decoder_l2', d_result['decoder_norm'], i + 1)


        if i % 50 == 0:
            '''val recon error'''
            val_err = predict(new_model_2, in_val_dl, device, flatten=True)
            
            in_pred = predict(new_model_2, in_test_dl, device, True)
            out_pred = predict(new_model_2, out_test_dl, device, True)
            auc = roc_btw_arr(out_pred, in_pred)
            writer.add_scalar('step3/AUROC', auc, i + 1)
            writer.add_scalar('step3/val_recon', val_err.mean().item(), i + 1)
            
        i += 1

torch.save(new_model_2.state_dict(), f'{result_dir}/step3.pkl')
